...
using STGTieredBuildAndRepair.Handlers;
...
namespace STGTieredBuildAndRepair.Models
{
    ...
    public class SyncBlockSettings
    {
        ...
        public void CheckLimits(NanobotSystem system, bool init)
        {
            ...
            // Per-tier overrides for the subtype-gated fields. Non-tier fields (Area*, Sound*,
            // color, multipliers) still come from the global Mod.Settings.Welder.
            var tier = TierConfig.ForBlock(system != null ? system.Welder : null);
            ...
            if (tier.UseGrindJanitorFixed || init)
            {
                UseGrindJanitorOn = tier.UseGrindJanitorDefault;
            ...
            if (tier.CollectIfIdleFixed || init) Flags = (Flags & ~Settings.ComponentCollectIfIdle) | (tier.CollectIfIdleDefault ? Settings.ComponentCollectIfIdle : 0);
            ...
            if ((tier.AllowedSearchModes & SearchMode) == 0 || init)
            {
                if ((tier.AllowedSearchModes & tier.SearchModeDefault) != 0)
                {
                    SearchMode = tier.SearchModeDefault;
                }
                else
                {
                    if ((tier.AllowedSearchModes & SearchModes.Grids) != 0) SearchMode = SearchModes.Grids;
                    else if ((tier.AllowedSearchModes & SearchModes.BoundingBox) != 0) SearchMode = SearchModes.BoundingBox;
                }
            }

            if ((tier.AllowedWorkModes & WorkMode) == 0 || init)
            {
                if ((tier.AllowedWorkModes & tier.WorkModeDefault) != 0)
                {
                    WorkMode = tier.WorkModeDefault;
                }
                else
                {
                    if ((tier.AllowedWorkModes & WorkModes.WeldOnly) != 0) WorkMode = WorkModes.WeldOnly;
                    else if ((tier.AllowedWorkModes & WorkModes.WeldBeforeGrind) != 0) WorkMode = WorkModes.WeldBeforeGrind;
                    else if ((tier.AllowedWorkModes & WorkModes.GrindBeforeWeld) != 0) WorkMode = WorkModes.GrindBeforeWeld;
                    else if ((tier.AllowedWorkModes & WorkModes.GrindIfWeldGetStuck) != 0) WorkMode = WorkModes.GrindIfWeldGetStuck;
                    else if ((tier.AllowedWorkModes & WorkModes.GrindOnly) != 0) WorkMode = WorkModes.GrindOnly;
                }
            }
        }
    }
