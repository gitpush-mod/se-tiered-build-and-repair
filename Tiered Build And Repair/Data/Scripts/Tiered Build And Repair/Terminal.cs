...
using STGTieredBuildAndRepair.Localization;
using STGTieredBuildAndRepair.Models;
using STGTieredBuildAndRepair.Terminal;
...
namespace STGTieredBuildAndRepair
{
    ...
    public static class NanobotTerminal
    {
        ...
        public static void InitializeControls()
        {
            lock (CustomControls)
            {
                ...
                try
                {
                    ...
                    // Init-time loosest-tier bools — passed to control creators that take bool params.
                    // Per-block tier filtering happens via the Func<> predicates below, which look up
                    // each block's tier in TierConfig at call-time. We pass true here so all controls
                    // get registered for the union of all tiers; visibility/enabled state then
                    // filters them per-block at runtime.
                    var weldingAllowed = true;
                    var grindingAllowed = true;
                    var janitorAllowed = true;
                    var janitorAllowedNoOwnership = true;
                    var janitorAllowedOwner = true;
                    var janitorAllowedFactionShare = true;
                    var janitorAllowedNeutral = true;
                    var janitorAllowedEnemies = true;
                    ...
                    Func<IMyTerminalBlock, bool> isWeldingAllowed = TierConfig.IsWeldingAllowed;
                    Func<IMyTerminalBlock, bool> isGrindingAllowed = TierConfig.IsGrindingAllowed;
                    Func<IMyTerminalBlock, bool> isJanitorAllowed = TierConfig.IsJanitorAllowed;
                    Func<IMyTerminalBlock, bool> isJanitorAllowedNoOwnership = (block) => TierConfig.IsJanitorAllowedForRelation(block, AutoGrindRelation.NoOwnership);
                    Func<IMyTerminalBlock, bool> isJanitorAllowedOwner = (block) => TierConfig.IsJanitorAllowedForRelation(block, AutoGrindRelation.Owner);
                    Func<IMyTerminalBlock, bool> isJanitorAllowedFactionShare = (block) => TierConfig.IsJanitorAllowedForRelation(block, AutoGrindRelation.FactionShare);
                    Func<IMyTerminalBlock, bool> isJanitorAllowedNeutral = (block) => TierConfig.IsJanitorAllowedForRelation(block, AutoGrindRelation.Neutral);
                    Func<IMyTerminalBlock, bool> isJanitorAllowedEnemies = (block) => TierConfig.IsJanitorAllowedForRelation(block, AutoGrindRelation.Enemies);
                    ...
                    {
                        var system = GetSystem(block);
                        return system != null && system.ComponentCollectPriority != null && system.ComponentCollectPriority.Selected != null && system.Settings.SearchMode == SearchModes.BoundingBox && TierConfig.IsCollectPriorityConfigurable(block);
                    };
                    ...
                    {
                        // --- Select search mode ---
                        // Across all tiers, AllowedSearchModes ranges from {Grids} (Base) to {Grids|BoundingBox} (Upgraded/Advanced).
                        // At least one tier supports both modes, so init-time onlyOneAllowed is false — the combobox stays
                        // active. Per-block tier filtering of the actual mode pick happens in the combobox setter (ComboBoxes.cs).
                        ...
                        // --- Select work mode ---
                        // Advanced tier has 5 work modes; at least one tier has more than one. Init-time onlyOneAllowed = false.
                    ...
                    // --- Grinding --- (whole section hidden on tiers without grinding)
                    label = Labels.Create("GrindingSettings", Texts.GrindSettings_Headline);
                    label.Visible = isGrindingAllowed;
                    {
                    ...
                    // --- Collecting --- (whole section hidden on tiers without Fly Mode)
                    label = Labels.Create("CollectingSettings", Texts.CollectSettings_Headline);
                    label.Visible = TierConfig.IsBoundingBoxSearchAllowed;
                    {
                        // --- Collect floating objects ---
                        {
                            ...
                            // Hide list on tiers where priorities can't be reordered (Base + Upgraded).
                            // Showing a read-only list when the user can't change anything is confusing.
                            listbox.Visible = (b) => isCollectPossible(b) && TierConfig.IsCollectPriorityConfigurable(b);
                            ...
                            // --- Push Ingot/ore immediately --- (hidden alongside Collecting section on no-Fly tiers)
                            onoffSwitch = OnOffSwitches.CreatePushIngotOreImmediately(isReadonly, isBaRSystem);
                            onoffSwitch.Visible = TierConfig.IsBoundingBoxSearchAllowed;
                    ...
                    // --- Highlight Area --- (separator hidden on tiers without Fly Mode)
                    separateArea = Separators.Create("SeparateArea", TierConfig.IsBoundingBoxSearchAllowed);
                    {
        ...
        {
            if (block.BlockDefinition.SubtypeName.StartsWith("STG_Nanobot"))
            {
