...
namespace STGTieredBuildAndRepair.Terminal
{
    public static class ComboBoxes
    {
        ...
        {
            // Per-tier enabled: combo is greyed out when the tier offers only one search mode.
            // Base = Grids only (greyed), Upgraded/Advanced = Grids|BoundingBox (active).
            Func<IMyTerminalBlock, bool> isEnabled = (block) =>
            {
                var allowed = TierConfig.ForBlock(block).AllowedSearchModes;
                var onlyOne = (allowed & (allowed - 1)) == 0;
                return !onlyOne && isBaRSystem(block);
            };
            ...
            var control = Create(
                ...
                (block, value) =>
                {
                    ...
                    if (system != null)
                    {
                        if (TierConfig.ForBlock(block).AllowedSearchModes.HasFlag((SearchModes)value))
                        {
        ...
        {
            // Per-tier enabled: combo greyed when tier offers only one work mode.
            // Base/Upgraded = WeldOnly only (greyed), Advanced = 5 modes (active).
            Func<IMyTerminalBlock, bool> isEnabled = (block) =>
            {
                var allowed = TierConfig.ForBlock(block).AllowedWorkModes;
                var onlyOne = allowed != 0 && (allowed & (allowed - 1)) == 0;
                return !onlyOne && isBaRSystem(block);
            };
            ...
            var control = Create(
                ...
                (block, value) =>
                {
                    ...
                    if (system != null)
                    {
                        if (TierConfig.ForBlock(block).AllowedWorkModes.HasFlag((WorkModes)value))
                        {
