...
namespace STGTieredBuildAndRepair.Terminal
{
    public static class OnOffSwitches
    {
        ...
        {
            // isJanitorAllowed already factors in per-tier UseGrindJanitorFixed + grinding/janitor allowed.
            var isEnabled = isJanitorAllowed;
            ...
            var control = Create(
                ...
                {
                    var system = NanobotTerminal.GetSystem(block);
                    if (system != null && !TierConfig.ForBlock(block).UseGrindJanitorFixed && isJanitorAllowed(block))
                    {
        ...
        {
            // Visible only on tiers where CollectIfIdle is configurable (Advanced). Hides the
            // control entirely on Base/Upgraded where the feature is locked off.
            Func<IMyTerminalBlock, bool> isVisible = TierConfig.IsCollectIdleConfigurable;
            Func<IMyTerminalBlock, bool> isEnabled = (block) =>
                !TierConfig.ForBlock(block).CollectIfIdleFixed && isCollectPossible(block);
            ...
            var control = Create(
                ...
                {
                    var system = NanobotTerminal.GetSystem(block);
                    if (system != null && !TierConfig.ForBlock(block).CollectIfIdleFixed)
                    {
        ...
        {
            // Visible only when the tier offers Fly Mode (BoundingBox) — area visualization is
            // only meaningful for the 3D bounding box, not Walk Mode's connected-grid traversal.
            Func<IMyTerminalBlock, bool> isVisible = TierConfig.IsBoundingBoxSearchAllowed;
        ...
        public static IMyTerminalControlOnOffSwitch CreateWeldPriority(Func<IMyTerminalBlock, bool> isWeldingAllowed, Func<IMyTerminalBlock, bool> isReadonly, Func<IMyTerminalBlock, bool> isBaRSystem)
        {
            ...
            {
                var system = NanobotTerminal.GetSystem(block);
                return system != null && system.BlockWeldPriority != null && system.BlockWeldPriority.Selected != null && isWeldingAllowed(block) && !TierConfig.ForBlock(block).PriorityFixed;
            };
            ...
            var control = Create(
                ...
                {
                    var system = NanobotTerminal.GetSystem(block);
                    if (system != null && system.BlockWeldPriority != null && system.BlockWeldPriority.Selected != null && isWeldingAllowed(block) && !TierConfig.ForBlock(block).PriorityFixed)
                    {
        ...
        {
            var isEnabled = isJanitorAllowedEnemies;
            ...
            var control = Create(
                ...
                {
                    var system = NanobotTerminal.GetSystem(block);
                    if (system != null && !TierConfig.ForBlock(block).UseGrindJanitorFixed && isJanitorAllowedEnemies(block))
                    {
        ...
        {
            var isEnabled = isJanitorAllowedNoOwnership;
            ...
            var control = Create(
                ...
                {
                    var system = NanobotTerminal.GetSystem(block);
                    if (system != null && !TierConfig.ForBlock(block).UseGrindJanitorFixed && isJanitorAllowedNoOwnership(block))
                    {
        ...
        {
            var isEnabled = isJanitorAllowedNeutral;
            ...
            var control = Create(
                ...
                {
                    var system = NanobotTerminal.GetSystem(block);
                    if (system != null && !TierConfig.ForBlock(block).UseGrindJanitorFixed && isJanitorAllowedNeutral(block))
                    {
        ...
        public static IMyTerminalControlOnOffSwitch CreateGrindPriority(Func<IMyTerminalBlock, bool> isGrindingAllowed, Func<IMyTerminalBlock, bool> isReadonly, Func<IMyTerminalBlock, bool> isBaRSystem)
        {
            var control = Create(
                ...
                {
                    var system = NanobotTerminal.GetSystem(block);
                    return system != null && system.BlockGrindPriority != null && system.BlockGrindPriority.Selected != null && isGrindingAllowed(block) && !TierConfig.ForBlock(block).PriorityFixed;
                },
                ...
                {
                    var system = NanobotTerminal.GetSystem(block);
                    if (system != null && system.BlockGrindPriority != null && system.BlockGrindPriority.Selected != null && isGrindingAllowed(block) && !TierConfig.ForBlock(block).PriorityFixed)
                    {
        ...
        public static IMyTerminalControlOnOffSwitch CreateWeldIgnorePriorityOrder(bool weldingAllowed, Func<IMyTerminalBlock, bool> isWeldingAllowed, Func<IMyTerminalBlock, bool> isReadonly, Func<IMyTerminalBlock, bool> isBaRSystem)
        {
            var isEnabled = weldingAllowed ? isBaRSystem : isReadonly;

            var control = Create(
                // Id:
                "WeldIgnorePriorityOrder",

                // Texts
                Texts.WeldIgnorePriority,
                Texts.WeldIgnorePriority_Tooltip,
                MySpaceTexts.SwitchText_On,
                MySpaceTexts.SwitchText_Off,
                ...
                {
                    var system = NanobotTerminal.GetSystem(block);
                    return system != null ? ((system.Settings.Flags & SyncBlockSettings.Settings.WeldIgnorePriorityOrder) != 0) : false;
                },
                ...
                (block, value) =>
                {
                    ...
                    {
                        system.Settings.Flags = (system.Settings.Flags & ~SyncBlockSettings.Settings.WeldIgnorePriorityOrder) | (value ? SyncBlockSettings.Settings.WeldIgnorePriorityOrder : 0);
                    }
                },
        ...
        public static IMyTerminalControlOnOffSwitch CreateCollectPriority(Func<IMyTerminalBlock, bool> isChangeCollectPriorityPossible, Func<IMyTerminalBlock, bool> isReadonly, Func<IMyTerminalBlock, bool> isBaRSystem)
        {
            var control = Create(
                ...
                {
                    var system = NanobotTerminal.GetSystem(block);
                    if (system != null && system.ComponentCollectPriority != null && system.ComponentCollectPriority.Selected != null && !TierConfig.ForBlock(block).CollectPriorityFixed)
                    {
